...
* **Structure-Free (Abstract) Workflow:** Exact same as above. `ChiralTransitionFinder(spg_number)` only requires the parent space group number.
...
* **How to find the MSG (With Structure):**
  1. Use `MagneticTransitionFinder(cell, mag_sites)` on the parent phase.
  2. Filter the output for `is_chiral == True`. This directly yields the physically realisable BNS number for those specific atomic sites.
* **Structure-Free (Abstract) Workflow:** Use `AbstractMagneticTransitionFinder(spg_number)`. It evaluates a generic time-odd order parameter transforming as the abstract irreps of the parent group, without needing a lattice. Filter the results for `is_chiral == True`.
...
* **Structure-Free (Abstract) Workflow:**
  1. Use `ChiralTransitionFinder(spg_number)` to find the structurally chiral intermediate Space Group number.
  2. Pass that intermediate SG number into `AbstractMagneticTransitionFinder(intermediate_spg_number)` to abstractly enumerate the final chiral Magnetic Space Groups. 
...
* **How to find the MSG (With Structure):**
  1. Use `MagneticTransitionFinder(cell)` on the parent phase to locate the intermediate physically realisable *achiral* MSG (e.g., $P\bar{1}'$).
  2. To find the final transition, treat this intermediate achiral MSG as the new parent phase.
  3. Run structural symmetry-breaking analysis using the operations of the achiral MSG to see which atomic distortions break the remaining $\bar{1}'$ symmetry. Their mathematical intersection defines the final chiral MSG.
* **Structure-Free (Abstract) Workflow:**
  1. Use `AbstractMagneticTransitionFinder(spg_number)` to find an intermediate abstract achiral Magnetic Space Group.
  2. To abstractly find the secondary structural transition, one must evaluate the time-even representations of the intermediate MSG and identify isotropy subgroups that break the remaining improper operations (note: direct API support for MSG parents in `ChiralTransitionFinder` is a complex future group-theoretic extension).
...
* **How to find the MSG (With Structure):**
  1. Calculate the real-space basis vectors for both the structural distortion and the magnetic order.
...
* **Structure-Free (Abstract) Workflow:** Pure group theory requires evaluating the isotropy subgroups of a *coupled* representation (the tensor product of a time-even structural irrep and a time-odd magnetic irrep) of the parent space group. The combined isotropy subgroup yields the final chiral MSG.
